RSpec.describe 'PR::Pin.charges.list' do
  ...
  context 'paginated requests' do
    it_behaves_like 'a_paginated_response' do
      let(:uri) { 'https://test-api.pinpayments.com/1/charges' }
      let(:records) do
        Array.new(total_count) { Factory.structs[:charge] }
      end
      let(:base_result) { PR::Pin.charges.list(params) }
    end
  end
end
