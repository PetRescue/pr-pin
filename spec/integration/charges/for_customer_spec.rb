RSpec.describe 'PR::Pin.charges.for_customer' do
  ...
  context 'single request' do
    ...
    let!(:request_stub) do
      stub_request(
        :get,
        "https://test-api.pinpayments.com/1/customers/#{customer.token}/charges"
      ).with(
        query: params,
    ...
    subject!(:result) do
      PR::Pin.charges.for_customer(customer.token, **params)
    end
  ...
  context 'paginated requests' do
    it_behaves_like 'a_paginated_response' do
      let(:uri) { "https://test-api.pinpayments.com/1/customers/#{customer.token}/charges" }
      let(:records) do
        Array.new(total_count) { Factory.structs[:charge] }
      end
      let(:base_result) { PR::Pin.charges.for_customer(customer.token, **params) }
    end
  end
  ...
  context 'error response' do
    ...
    let!(:request_stub) do
      stub_request(
        :get,
        "https://test-api.pinpayments.com/1/customers/#{record.token}/charges"
      ).with(
        query: params,
        basic_auth: [PIN_SECRET_KEY, ''],
        headers: {
    ...
    subject!(:error) do
      PR::Pin.charges.for_customer(record.token, **params)
    end
