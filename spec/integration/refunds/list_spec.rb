RSpec.describe 'PR::Pin.refunds.list' do
  ...
  context 'single request' do
    ...
    let!(:request_stub) do
      stub_request(
        :get,
        'https://test-api.pinpayments.com/1/refunds'
      ).with(
        query: params,
  ...
  context 'paginated requests' do
    it_behaves_like 'a_paginated_response' do
      let(:uri) { 'https://test-api.pinpayments.com/1/refunds' }
      let(:records) do
        Array.new(total_count) { Factory.structs[:refund] }
      end
      let(:base_result) { PR::Pin.refunds.list(**params) }
    end
  end
end
